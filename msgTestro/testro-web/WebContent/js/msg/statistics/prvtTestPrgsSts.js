...
function colNameData(){
	...
	if($('#searchGubun').val() == "01"){
		colName =  ['거래코드','테스터(id)','테스트단계','업무구분','테스터','테스트단계','거래명(거래코드)','수행건수','성공건수','실패건수','성공율(%)','테스터','테스트단계','거래명(거래코드)','수행건수','성공건수','실패건수','성공율(%)'];
	}else if($('#searchGubun').val() == "02"){
		colName =  ['거래코드','테스터(id)','테스트단계','업무구분','테스터','테스트단계','거래명(거래코드)','수행건수','성공건수','실패건수','성공율(%)','테스터','테스트단계','수행건수','성공건수','실패건수','성공율(%)'];
	}
	return colName;
}
...
function getColModel(){
	...
	if($('#searchGubun').val() == "01"){
		colModel = [
				...
				{name:'testStgeName',index:'testStgeName', hidden:true},
				{name:'tempTranName',index:'tranName' ,summaryType:'count',summaryTpl :'합계 ({0}건)', hidden:true},
				...
				{name:'tempTranName',index:'tranName' ,summaryType:'count',summaryTpl :'합계 ({0}건)', width:350,align:"left",sortable:true},
				...
				{name:'tscsPercent',index:'tscsPercent' , width:50,formatter:'number',summaryType:myAvg2,align:"center",sortable:true}];
	}else if($('#searchGubun').val() == "02"){
	 colModel = [
				{name:'tranCd',index:'tranCd' , hidden:true},
			    {name:'tstrId',index:'tstrId' , hidden:true},
	...
	function JQGridReload(beUnload){
		...
		jQuery("#list2").jqGrid({
			...
			loadComplete:function(xhr){
			    ...
			    $("#list2").footerData('set',{tstrName:'합계', tscsExeCnt:tscsExeCntSum, tscsSucssCnt:tscsSucssCntSum,tscsMissCnt:tscsMissCntSum, tscsPercent:tscsPercent}); // 컬럼name:보여줄 텍스트
			    reloadGridSize();
			 },
...
function getList(){
	...
	tranUrl = "msg.statistics.getPrvtTestPrgsSts.do";
...
//엑셀다운로드 
function downExcelprvtTestPrgsSts() { 
	...
	if($("#searchGubun").val() == "01")       // 테스트 케이스
		gridCustomExportExcel('list2', [0,1,2,3,11,12,13,14,15,16,17], '개인별 진척현황',callback);
	else if($("#searchGubun").val() == "02")  // 시나리오
		gridCustomExportExcel('list2', [0,1,2,3,6,11,12,13,14,15,16,17], '개인별 진척현황',callback);
} 

// get방식 파라메터 얻기.
function getParam(){
	var tstrName = $("#tstrName").val();
	var connSevrDstcd = $("#connSevrDstcd").val();
	param = 'connSevrDstcd='+connSevrDstcd+'&tstrName='+encodeURIComponent(tstrName)+'&searchGubun='+$("#searchGubun").val()+'&testStartYMS='+$("#testStartYMS").val()+'&testEndYMS='+$("#testEndYMS").val()+'&projNo='+encodeURIComponent($("#projNo").val());
	return param;
}

// post data 얻기.
function getPostData(){
	var postData = {connSevrDstcd:$("#connSevrDstcd").val(),tstrName:$("#tstrName").val(),searchGubun:$("#searchGubun").val(),testStartYMS:$("#testStartYMS").val(),testEndYMS:$("#testEndYMS").val(),projNo:$("#projNo").val(),testStgeName:$("#testStgeName2").val()};
	return postData;
}
