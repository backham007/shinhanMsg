...
dest[i]=src[i];};Zapatec.Utils.__wch_id=0;Zapatec.Utils.createWCH=function(oEl){if(!Zapatec.is_ie||Zapatec.is_ie5){return null;}
...
Zapatec.is_opera=false;
Zapatec.is_ie=true;
Zapatec.is_ie5=false;
Zapatec.is_ie7=false;
Zapatec.is_mac_ie=false;
Zapatec.is_khtml=false;
Zapatec.is_konqueror=false;
Zapatec.is_gecko=false;
Zapatec.is_webkit=false;
Zapatec.webkitVersion=false;

if(!Object.prototype.hasOwnProperty){Object.prototype.hasOwnProperty=function(strProperty){try{var objPrototype=this.constructor.prototype;while(objPrototype){if(objPrototype[strProperty]==this[strProperty]){return false;}
...
this.element.style.display="none";Zapatec.Utils.hideWCH(this.WCH);this.hidden=true;this.hideShowCovered();};Zapatec.Calendar.prototype.showAt=function(x,y){var s=this.element.style;s.left=x+"px";s.top=y+"px";this.show();};
...
Zapatec.Calendar.prototype.showAtElement=function(el,opts){
  //ibsheet를 위한 수정
  if(el.classid=="CLSID:341FBC5F-2AE4-41B8-BFE5-A03170569A27") {
    var point = fGetXY(el);
    //정말 이상한데, 여기서 el.RowTop을 하지 않으면 0이 나온다.
    gLeft= el.ColLeft(el.MouseCol)+point.x; 
    gTop=el.RowTop(el.MouseRow) + el.RowHeight(el.MouseRow)+point.y;
    if (el.CountPosition == 1 || el.CountPosition == 2) gTop+=13; 
    window.status = "gLeft=" + gLeft + ",gTop=" + gTop  + ",el.offsetTop=" + el.offsetTop ;
  }
  var self=this; 
  var p;p=Zapatec.Utils.getElementOffset(el);if(!opts||typeof opts!="string"){this.showAt(p.x,p.y+el.offsetHeight);return true;}
  ...
  p.width=w;p.height=h;self.monthsCombo.style.display="none";Zapatec.Utils.fixBoxPosition(p,10);
  //ibsheet를 위한 수정
  if(el.classid=="CLSID:341FBC5F-2AE4-41B8-BFE5-A03170569A27"){
    p.x=gLeft;
    p.y=gTop; 
    window.status += "///////////////// gLeft=" + gLeft + ",gTop=" + gTop;
  }
  self.showAt(p.x,p.y);
};
...
if(params.saveDate){param_default("cookiePrefix",window.location.href+"--"+params.button.id);var cookieName=params.cookiePrefix;var newdate=Zapatec.Utils.getJCookie(cookieName);if(newdate!=null){document.getElementById(params.inputField.id).value=newdate; }}
...

// $Id: popupcalendar.js,v 1.1 2008/08/28 00:09:56 ssoh Exp $
// ** I18N
...
{
    var obj_receiver;
    obj_receiver = popUpCalendar.arguments;

    var date_formate;
    var time_format;
    var bool_showtime = false;
    
    var dateformatChar = obj_receiver[2].substring(4,5);
    
    date_formate = "%Y"+dateformatChar+"%m"+dateformatChar+"%d";
    //org_date       = date_formate;
    if(obj_receiver[2].length >10){

        time_format = obj_receiver[2].substring(10,obj_receiver[2].length);
        date_formate = date_formate+time_format;
        bool_showtime = true;
    }
    if(obj_receiver[2].length == 8){
        date_formate = "%Y%m%d";
        bool_showtime = true
    }
    var getDaFormat,getDaTabNm;
    if(obj_receiver.length > 5){
        //alert(obj_receiver.length+"__"+obj_receiver[5]+"__"+obj_receiver[6]);
        getDaFormat = obj_receiver[5];
        getDaTabNm  = obj_receiver[6];
        //alert(getDaFormat+"____"+getDaTabNm.name);
    }
    
    var getFunc;
    if(obj_receiver.length > 6){
        getFunc = obj_receiver[7];
    }
    
    Zapatec.Calendar.setup({
                                firstDay          : 1,
                                showsTime         : bool_showtime,
                                electric          : false,
                                inputField        : obj_receiver[1],
                                button            : obj_receiver[0],
                                ifFormat          : date_formate,
                                displayArea       : getDaTabNm,
                                daFormat          : getDaFormat,
                                onUpdate          : getFunc
                               // timeInterval      : 01
                              });
    obj_receiver[0].onclick();

}

//ibsheet를 위한 수정
function hideCalendar() {
  if(window.calendar!=null){window.calendar.callCloseHandler();}
}