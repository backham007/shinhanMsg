	...
	return this.each( function() {
		...
		addCell = function(rowId,cell,pos,irow, srvr) {
			...
			if(v != "&#160;"){
				for(var charIndex = 0 ; charIndex < v.length ; charIndex++){
					if(v.charAt(charIndex) == ' '){
						vl += '&nbsp;';
					} else {
						vl += v.charAt(charIndex);
					}
				}
			}
			
			//return "<td role=\"gridcell\" "+prp+">"+v+"</td>";
			return "<td role=\"gridcell\" "+prp+">"+vl+"</td>";
		},
		...
		setPager = function (pgid, tp){
			...
			if(ts.p.pgbuttons===true) {
				...
				pgl += "<td id='"+po[0]+"' class='ui-pg-button ui-corner-all ui-state-default'><span class='ui-icon ui-icon-seek-first'></span></td>";
				pgl += "<td id='"+po[1]+"' class='ui-pg-button ui-corner-all ui-state-default'><span class='ui-icon ui-icon-seek-prev'></span></td>";
				pgl += pginp !== "" ? sep+pginp+sep:"";
				pgl += "<td id='"+po[2]+"' class='ui-pg-button ui-corner-all ui-state-default'><span class='ui-icon ui-icon-seek-next'></span></td>";
				pgl += "<td id='"+po[3]+"' class='ui-pg-button ui-corner-all ui-state-default'><span class='ui-icon ui-icon-seek-end'></span></td>";
...
$.jgrid.extend({
						...
						if ( nm !== 'cb' && nm !== 'subgrid' && nm !== 'rn') {
							if($t.p.treeGrid===true && nm == $t.p.ExpandColumn) {
								...
								try {
									...
									//IE7,IE8에서 공백처리 오류로 수정 (KJH)
									var v = '';
									for(var charIndex = 0 ; charIndex < res[nm].length ; charIndex++){
										if(res[nm].charAt(charIndex) == '&'){
											if(res[nm].substring(charIndex,charIndex+6) == '&nbsp;'){
												v += ' ';
												charIndex = charIndex + 5; 
											} else {
												v += res[nm].charAt(charIndex);
											}
										} else {
											v += res[nm].charAt(charIndex);
										}
									...
									res[nm] = v;
									/*
									if($t.p.colModel[i].edittype == 'textarea'){
										res[nm] = $.unformat(this,{rowId:ind.id, colModel:$t.p.colModel[i]},i,true);	//두줄 데이터 입력에 문제가 있어 수정
									} else {
										res[nm] = $.unformat(this,{rowId:ind.id, colModel:$t.p.colModel[i]},i);
									}*/
								} catch (e){
					...
					$(this.p.colModel).each(function(i){
						...
						if( data[nm] !== undefined) {
							...
							if(t.p.treeGrid===true && nm == t.p.ExpandColumn) {
								...
								if(vl != "&#160;"){
									//IE7,IE8에서 공백처리 오류로 수정 (KJH)
									for(var charIndex = 0 ; charIndex < vl.length ; charIndex++){
										if(vl.charAt(charIndex) == '&'){
											if(vl.substring(charIndex,charIndex+6) == '&nbsp;'){
												v += ' ';
												charIndex = charIndex + 5; 
											} else {
												v += vl.charAt(charIndex);
											}
										} else {
											v += vl.charAt(charIndex);
										}
								...
								$("td:eq("+i+")",ind).text(v).attr(title);	//&문자처리 오류로 수정 (KJH)
							}
						}
					});
					if(t.p.datatype == 'local') {
			...
			this.each(function() {
				...
				while(k < datalen) {
					...
					for(i = gi+si+ni; i < t.p.colModel.length;i++){
						...
						row += "<td role=\"gridcell\" aria-describedby=\""+t.p.id+"_"+nm+"\" "+prp+">"+vl+"</td>";
					}
		...
		return this.each(function() {
			...
			$(this.p.colModel).each(function(i) {
				if ($.inArray(this.name,colname) !== -1 && this.hidden === sw) {
					...
					this.hidden = !sw;
					fndh=false; //��������  origin(false)  �׸��� ���̰�  �÷� hide�ÿ� �ڵ�����Ǿ� �����Ͽ����� �ٸ��÷��� �þ�� ������ ����. 
				}
			});
			if(fndh===true) {
					...
					if(nData !== "" || forceupd === true) {
						...
						if($t.p.treeGrid && $(".tree-wrap",$(tcell)).length>0) {
							...
							if(v != "&#160;"){
								...
								for(var charIndex = 0 ; charIndex < v.length ; charIndex++){
									if(v.charAt(charIndex) == '&'){
										if(v.substring(charIndex,charIndex+6) == '&nbsp;'){
											vl += ' ';
											charIndex = charIndex + 5; 
										} else {
											vl += v.charAt(charIndex);
							...
							$(tcell).text(vl).attr(title);	//&문자처리 오류로 수정 (KJH)
	...
	//공백처리 오류로 setCell function을 수정하여 html을 셀에 세팅할 수 있게 기존로직을 제공 (KJH)
	setCellHtml : function(rowid,colname,nData,cssp,attrp, forceupd) {
		return this.each(function(){
			var $t = this, pos =-1,v, title;
			if(!$t.grid) {return;}
			if(isNaN(colname)) {
			...
			if(pos>=0) {
				...
				if(ind) {
					try {
						...
						//IE7,IE8에서 공백처리 오류로 수정 (KJH)
						var v = '';
						for(var charIndex = 0 ; charIndex < ret.length ; charIndex++){
							if(ret.charAt(charIndex) == '&'){
								if(ret.substring(charIndex,charIndex+6) == '&nbsp;'){
									v += ' ';
									charIndex = charIndex + 5; 
								} else {
									v += res[nm].charAt(charIndex);
	...
	setFrozenColumns2 : function () {
		return this.each(function() {
			...
			if( maxfrozen>=0 && frozen) {
				...
				//$t.grid.fhDiv = $('<div style="position:absolute;left:0px;top:'+top+'px;height:auto;" class="frozen-div ui-state-default ui-jqgrid-hdiv"></div>');
				$t.grid.fhDiv = $('<div style="position:absolute;left:0px;top:'+top+'px;height:40px;z-index:10000;" class="frozen-div ui-state-default ui-jqgrid-hdiv"></div>');
				$t.grid.fbDiv = $('<div style="position:absolute;left:0px;top:41px;overflow-y:hidden" class="frozen-bdiv ui-jqgrid-bdiv"></div>');
				$("#gview_"+$.jgrid.jqID($t.p.id)).append($t.grid.fhDiv);
				var htbl = $(".ui-jqgrid-htable","#gview_"+$.jgrid.jqID($t.p.id)).clone(true);
				// groupheader support - only if useColSpanstyle is false
				if($t.p.groupHeader) {
			...
			if (editable == "0" && !$(ind).hasClass("not-editable-row")) {
				...
				$('td[role="gridcell"]',ind).each( function(i) {
					...
					if(treeg) { tmp = $("span:first",this).html();}
						...
						try {
							//tmp =  $.unformat(this,{rowId:rowid, colModel:cm[i]},i);
							tmp =  $.unformat(this,{rowId:rowid, colModel:cm[i]},i,true);	//IE7,IE8에서 공백처리 오류로 수정 (KJH)
							/*
							if(cm[i].edittype == 'textarea'){
								tmp =  $.unformat(this,{rowId:rowid, colModel:cm[i]},i,true);	//두줄 데이터 입력에 문제가 있어 수정 (KJH)
							} else {
								tmp =  $.unformat(this,{rowId:rowid, colModel:cm[i]},i);
							}*/
						} catch (_) {
				...
				if(cnt > 0) {
					...
					$(ind).attr("editable","1");
					//$("td:eq("+focus+") input",ind).focus();	//포커스 이동문제로 주석처리 editRow후 직접코딩하여 포커스이동 (KJH)
					if(o.keys===true) {
				...
				$.ajax($.extend({
					...
					success: function(data,stat,res){
						...
						if( $.isFunction(o.successfunc)) { 
							sucret = o.successfunc.call($t, data);
							if($.isArray(sucret)) {
...
Map.prototype = {   
    ...
    // 2011.09.23 : 해당값을 가진 첫번째 키를 구하는 로직 추가
...
//그리드 날짜포맷터
function gridDateFmatter ( cellvalue, options, rowObject ) {
	
	if (!cellvalue) return '';
	var newformat = options.colModel.formatoptions.newformat;
	var srcformat = options.colModel.formatoptions.srcformat;
	return dateFmatter(cellvalue, srcformat, newformat);
}

//그리드 날짜 언포맷터
function gridDateUnFmatter(cellvalue, options, rowObject) {
	if (!cellvalue) return '';
	var newformat = options.colModel.formatoptions.newformat;
	var srcformat = options.colModel.formatoptions.srcformat;
	return dateFmatter(cellvalue, newformat, srcformat);
}


// 날짜 포맷터
function dateFmatter(cellvalue, srcformat, newformat) {
	...
	for (var i = 0; i < srcformat.length; i++) {
		var char = srcformat.substring(i, i+1);
		if (char) {
			if (char=='y') {
				y = cellvalue.substring(index,index+4);
				index = index+4;
			} else if (char=='M') {
				M = cellvalue.substring(index,index+2);
				index = index+2;
			} else if (char=='d') {
				d = cellvalue.substring(index,index+2);
				index = index+2;
			} else if (char=='h') {
				h = cellvalue.substring(index,index+2);
				index = index+2;
			} else if (char=='m') {
				m = cellvalue.substring(index,index+2);
				index = index+2;
			} else if (char=='s') {
				s = cellvalue.substring(index,index+2);
				index = index+2;
			} else {
				index = index+1;
			}
	...
	if (y) newformat = newformat.split('y').join(y);
	if (M) newformat = newformat.split('M').join(M);
	if (d) newformat = newformat.split('d').join(d);
	if (h) newformat = newformat.split('h').join(h);
	if (m) newformat = newformat.split('m').join(m);
	if (s) newformat = newformat.split('s').join(s);
	return newformat;
}

// codeFomatter
var codeMap = new Map();
function gridCodeFmatter( cellvalue, options, rowObject ){
	var output;
	if (!cellvalue) return '';
	var cdclMnName = options.colModel.formatoptions.cdclMnName;
	if (!cdclMnName) return cellvalue;
	
	if(codeMap.containsKey(cdclMnName)) {
		var codeList = codeMap.get(cdclMnName);
		
		for(var i = 0 ; i < codeList.length ; i++){
			if(cellvalue == codeList[i].dtailCd) {
				return codeList[i].dtailName;
			}
		}
		return cellvalue;
	}
	...
	$.ajax({
		...
		data:{"useYN":"Y","sidx":"OUTPTSEQNO","cdclMnName":cdclMnName},
		success: function(data){
			codeMap.put(cdclMnName, data.list);
			for(var i = 0 ; i < data.list.length ; i++){
				if(cellvalue == data.list[i].dtailCd) {
					output = data.list[i].dtailName;
				}
			}
		},
...
//codeUnFomatter
function gridCodeUnFmatter( cellvalue, options, rowObject ){
	if (!cellvalue) return '';
	var cdclMnName = options.colModel.formatoptions.cdclMnName;
	if (!cdclMnName) return cellvalue;
	...
	if(codeMap.containsKey(cdclMnName)) {
		...
		for(var i = 0 ; i < codeList.length ; i++){
			if(cellvalue == codeList[i].dtailName) {
				return codeList[i].dtailCd;
			}
		}
		return cellvalue;
	}
...
// grid excel up/down 초기화
function gridExcelInit(domain) {
	if($('#exportExcelForm').length == 0){
		$('<form />').attr({
		    id: 'exportExcelForm',
		    method : 'post',
		    action : 'gridExcelService'
		}).appendTo("body");
	
		$('<input type="hidden" />').attr({
			name: 'excelContents',
			id : 'excelContents'
		}).appendTo($('#exportExcelForm'));
		$('<input type="hidden" />').attr({
			name: 'sheetName',
			id : 'sheetName'
		}).appendTo($('#exportExcelForm'));
...
// 엑셀내려받기
function gridExportExcel(gridId, excludeColumnIdx, sheetName) {
	var columnNames = $('#' + gridId).jqGrid('getGridParam','colNames');
	if (jQuery('#' + gridId).jqGrid('getGridParam','rownumbers')) {
	...
	// 멀티 셀렉터일때는 첫번째 컬럼을 무시한다.
	if (jQuery('#' + gridId).jqGrid('getGridParam','multiselect')) {
		var tempArray = new Array();
		for (var i in columnNames) {
			if (i==0) continue;
			tempArray.push(columnNames[i]);
	...
	for (var i in columnNames) {
		var isExclude = false;
		for (var j in excludeColumnIdx) {
			if (i==excludeColumnIdx[j]) {
				isExclude = true;
				break;
			}
		}
		if (!isExclude) newColumnNames.push(columnNames[i].split('<br/>').join('  '));
	}
	
	// 서버에 올리엑셀 데이터
	var excelContents = new Array();
	excelContents.push(newColumnNames);
	
	var dataIds=new Array();
	dataIds=$('#' + gridId).getDataIDs();
    var rowData=$('#' + gridId).getRowData(dataIds[0]);
    var colNames=new Array();  
    var colIdx=0;
    var rowDataCnt=0;
    for (var i in rowData){
    	var isExclude = false;
    	for (var k in excludeColumnIdx) {
    ...
    for(var i in dataIds) {
        var data=$('#' + gridId).getRowData(dataIds[i]);
        var rows = new Array();
        for(var j in colNames) {
        	rows.push(data[colNames[j]]);
        }
        excelContents.push(rows);
    }


    var postData = JSON.stringify(excelContents); 

	$('#excelContents')[0].value=postData;
	$('#sheetName')[0].value=sheetName;
//	$('#sheetName')[0].value="GRID_DATA";
	$('#exportExcelForm').submit();
}


//커스톰엑셀내려받기
function gridCustomExportExcel(gridId, excludeColumnIdx, sheetName, callback) {
...
// 엑셀올리기
function gridImportExcel(mappingExcel, limitRowCnt, tcDataMaxCnt) {
	$.ajaxFileUpload(
		{
			url:'cmn.service.excelUpload.do',
			secureuri:false,
			...
			success: function (data, status) {
			 	if (data.errCd) alert('에러코드 : ' + data.errCd + "\n에러메시지 : " + data.errMsg);
	  			...
	  			if (limitRowCnt && result.length > limitRowCnt) {
	  				alert("테스트데이터의 총 갯수는 "+tcDataMaxCnt+"건을 초과할 수 없습니다");
	  				return;
	  			}
		  		for (var i=0; i<result.length; i++) {
		  			mappingExcel(result[i], i, result.length - 1);
		  		}
		  		
		  		setTimeout("window.close()", 1000);
		    },
		    error: function (data, status, e) {
		    	 alert("엑셀업로드중에 오류가 발생하였습니다.[" + error + "]"); 
			}
		});
}
