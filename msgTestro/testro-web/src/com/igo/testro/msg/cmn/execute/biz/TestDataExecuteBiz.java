			...
			for(CheckPointDTO checkPointDTO : checkPointDTOs){
				...
				for( ; i < outputDataDetailDtos.size() ; i++){
					...
					if(SUCSSYN[0].equals(rptCheckPointDTO.getChkYN())){
						...
						String fldData = outputDataDetailDto.getTsdataFldData().trim();	    // 출력값
						String chkPointExpcCtnt = rptCheckPointDTO.getChkPointExpcCtnt();	// 기대값
						String firstCtnt = chkPointExpcCtnt;
						String lastCtnt = chkPointExpcCtnt;
						try{
							if(FLDTYPE[1].equals(outputDataDetailDto.getTscsFldType())){
								if(chkPointExpcCtnt.indexOf(MARK[0]) != -1){
									String[] chkPointExpcCtnts = chkPointExpcCtnt.split(MARK[0]);
									firstCtnt = chkPointExpcCtnts[0].trim();
									lastCtnt = chkPointExpcCtnts[chkPointExpcCtnts.length-1].trim();
								}
								if(outputDataDetailDto.getTscsFldSizeCnt().indexOf(".") == -1){
									...
									int nFldData = Integer.valueOf(fldData);
									if(Integer.valueOf(firstCtnt) <= nFldData && nFldData <= Integer.valueOf(lastCtnt)){
									...
									double dFldData = Double.valueOf(fldData);
									if(Double.valueOf(firstCtnt) <= dFldData && dFldData <= Double.valueOf(lastCtnt)){
										rptCheckPointDTO.setChkPointSucssYN(SUCSSYN[0]);
									}else{
										rptCheckPointDTO.setChkPointSucssYN(SUCSSYN[1]);
										successYn = SUCSSYN[1];
								...
								if(chkPointExpcCtnt != null && chkPointExpcCtnt.trim().equals(fldData)){
									rptCheckPointDTO.setChkPointSucssYN(SUCSSYN[0]);
								} else if(chkPointExpcCtnt == null && "".equals(fldData)){
									rptCheckPointDTO.setChkPointSucssYN(SUCSSYN[0]);
								} else {
									rptCheckPointDTO.setChkPointSucssYN(SUCSSYN[1]);
									successYn = SUCSSYN[1];
