...
import com.igo.testro.msg.tcmng.dao.TcmngInfoDao;
 ...
 * </p>
 */
public class TcmngInfoBiz {
	final ITestroLogger logger = TestroLogHelper.getBiz();
	
	@Autowired
	private TcmngInfoDao tcmngInfoDao;
	...
	public Map getTcList(HttpServletRequest request) {
		try{
			...
			//테스트케이스 총건수
			int totCnt = tcmngInfoDao.getTcTotCnt(param);
			
			//테스트케이스 리스트
			ArrayList<TcDto> tcList = tcmngInfoDao.getTcList(param);
	...
	public Map getTdList(HttpServletRequest request) {
		try{
			...
			//테스트데이터 총건수
			int totCnt = tcmngInfoDao.getTdTotCnt(param);
			
			//테스트데이터 리스트
			ArrayList<TestDataDto> tdList = tcmngInfoDao.getTdList(param);
	...
	public TestDataDto getTdDetailList(HttpServletRequest request) {
		try{
			...
			tdDto.setHeaderList(tcmngInfoDao.getTdDetailList(param));
			//개별부
			param.put("tscsFldDiv", "02");
			tdDto.setInviList(tcmngInfoDao.getTdDetailList(param));
