...
//테스트케이스 목록
	...
	jQuery("#list").jqGrid({
	    ...
	    colModel:[
					{name: 'rsultsucssyn',index:'rsultsucssyn', width:80, align:"center", hidden: true, sortable:false},
					...
					{name: 'chkYNVal',index: 'ChkYNVal', width:80, sortable:false, hidden:true},
					${colModel}
	    ],
	...
	for(var i = 0; i < colData.length; i++){
		jQuery('#list').jqGrid('addRowData', i ,colData[i]);
		//체크포인트 밑줄, 손가락 표시
		jQuery('#list').find("#" + i + " td[aria-describedby='list_chkYN']").css('cursor','pointer').css('text-decoration','underline').css('font','bold');
		//반복부 밑줄, 손가락 표시
		$.each(rptCol, function (index, value) {
			jQuery('#list').find("#" + i + " td:eq("+(parseInt(value)+1)+")").css('cursor','pointer').css('text-decoration','underline').css('font','bold');
		});
		
		//jQuery("#list").jqGrid('setCellHtml', i, "chkYN", "<a style='cursor:pointer;text-decoration: underline;'>"+colData[i].chkYN+"</a>");
	}
